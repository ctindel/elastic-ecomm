...
def generate_image_for_product(product):
    ...
    logger.info(f"Generating image for product ID: {product['id']} - {product['name']}")
    ...
    while True:  # Loop forever until we succeed or explicitly return
        try:
            ...
            logger.info(f"Success! Image saved for product ID: {product['id']} to {image_path}")
            return str(image_path)
            ...
            if hasattr(e, 'response') and e.response and e.response.status_code == 429:
                ...
                logger.warning(f"Rate limit hit for product ID: {product['id']}. Retrying in {backoff_time:.2f} seconds... (Attempt {attempt+1}/∞)")
                time.sleep(backoff_time)
            else:
                # For other request errors, log and retry with a delay
                logger.error(f"Error generating image for product ID: {product['id']}: {e}")
                if hasattr(e, 'response') and e.response:
                    logger.error(f"Response: {e.response.text}")
                # Wait before retrying
                backoff_time = min(10 + random.uniform(0, 5), 30)
                logger.warning(f"Request error for product ID: {product['id']}. Retrying in {backoff_time:.2f} seconds...")
                time.sleep(backoff_time)
                
        except Exception as e:
            # For unexpected errors, log and retry with a delay
            logger.error(f"Unexpected error generating image for product ID: {product['id']}: {e}")
            # Wait before retrying
            backoff_time = min(10 + random.uniform(0, 5), 30)
            logger.warning(f"Unexpected error for product ID: {product['id']}. Retrying in {backoff_time:.2f} seconds...")
            time.sleep(backoff_time)
